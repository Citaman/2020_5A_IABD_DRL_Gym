...
class ExpertApprenticeAgent(Agent):
    ...
    def act(self, gs: GameState) -> int:
        ...
        if self.apprentice_training_count > self.apprentice_training_before_takeover:
            return gs.get_available_actions(
                gs.get_active_player())[np.argmax(self.brain.predict(np.array([gs.get_vectorized_state()]))[0][gs.get_available_actions(
        ...
        if root_hash not in memory:
            ExpertApprenticeAgent.create_node_in_memory(memory, root_hash, gs.get_available_actions(
        ...
        for i in range(self.max_iteration):
            ...
            while not gs_copy.is_game_over() and all((edge['n'] > 0
                ...
                s = gs_copy.get_unique_id()
                if s not in memory:
                    ExpertApprenticeAgent.create_node_in_memory(memory, s, gs_copy.get_available_actions(
