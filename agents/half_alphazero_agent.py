...
from brains import AlphaQNetwork
from contracts import Agent, GameState


class HalfAlphaZeroAgent(Agent):
    def __init__(self, max_iteration: int, action_space_size: int, keep_memory: bool = True):
        self.max_iteration = max_iteration
        self.keep_memory = keep_memory
        self.memory = dict()
        self.action_space_size = action_space_size
        self.brain = AlphaQNetwork(output_dim=action_space_size, hidden_layers_count=5, neurons_per_hidden_layer=64)
    ...
    @staticmethod
    def create_node_in_memory(memory, node_hash, available_actions, current_player, q_values):
        memory[node_hash] = \
            [
                {
                    'r': 0,
    ...
    def act(self, gs: GameState) -> int:
        ...
        if root_hash not in memory:
            ...
            HalfAlphaZeroAgent.create_node_in_memory(memory, root_hash, gs.get_available_actions(
                gs.get_active_player()), gs.get_active_player(), q_values
                                                     )
        ...
        for i in range(self.max_iteration):
            ...
            while not gs_copy.is_game_over() and all((edge['n'] > 0
                ...
                if s not in memory:
                    ...
                    HalfAlphaZeroAgent.create_node_in_memory(memory, s, gs_copy.get_available_actions(
        ...
        chosen_action = max((edge for edge in memory[root_hash]), key=lambda e: e['n'])['a']
        ...
        self.actions_buffer.append(to_categorical(chosen_action, gs.get_action_space_size()))
        self.intermediate_reward = 0.0
    ...
    def observe(self, r: float, t: bool, player_index: int):
        ...
        if t:
            ...
            self.states_batch = np.concatenate((self.states_batch, states)) if self.states_batch is not None else states
            self.actions_batch = np.concatenate(
                (self.actions_batch, chosen_actions)) if self.actions_batch is not None else chosen_actions
            self.gains_batch = np.concatenate((self.gains_batch, gains)) if self.gains_batch is not None else gains

            if len(self.states_batch) > 1024:
                self.brain.train(self.states_batch, self.actions_batch, self.gains_batch)
                self.states_batch = None
                self.actions_batch = None
                self.gains_batch = None
                self.memory = dict()
